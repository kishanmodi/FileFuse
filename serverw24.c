...
#include <strings.h>       // For string operations
...
struct FileInfo *file_list[1024000];
int file_count = 0;
...
int create_tar_and_send(int new_socket) {
    ...
    char buffer[1024];                                                     // Buffer to store the tar file data
    size_t bytesRead;                                                      // Bytes read from the tar file
    ...
    sprintf(deleteCommand, "rm -rf %s %s", tempDirFullPath, tarFilePath);  // Delete the temporary directory and the tar file
    system(deleteCommand);                                                 // Execute the delete command
    ...
    *token_count = 0;                    // Initialize the token count
    while (token != NULL && *token_count < MAX_TOKEN_COUNT) {
        ...
        if (command_tokens[*token_count] == NULL) {
            // Handle allocation failure
            fprintf(stderr, ALLOCATION_ERROR);  // Print the error message
    ...
    return command_tokens;                // Return the command tokens array
}
...
void crequest(int new_socket) {
    ...
    while (1) {
        ...
        int mkdirReturn = system(mkdirCommand);  // Create the temporary directory
        if (mkdirReturn != 0) {
        ...
        printf("Received command: %s from socket: %d\n", client_code, new_socket);  // Print the command received from the client

        int token_count = 0;
        char **command_tokens = tokenizer(client_code, &token_count);  // Tokenize the command
        ...
        if (strcmp(command_tokens[0], "dirlist") == 0) {
            ...
            // Single File Search
            if (file_count == 0) {                              // If the file is not found
                sprintf(message, FILE_NOT_FOUND_ERROR);         // File not found
                send(new_socket, message, strlen(message), 0);  // Send the message to the client
                memset(message, 0, MAX_RESPONSE_LENGTH);        // Clear the message buffer
            } else {
                // Send File Details
                for (int i = 0; i < file_count; i++) {
                    ...
                    send(new_socket, message, strlen(message), 0);  // Send the file details to the client
                    memset(message, 0, MAX_RESPONSE_LENGTH);        // Clear the message buffer
                }

                send(new_socket, COMPLETE_MESSAGE, strlen(COMPLETE_MESSAGE), 0);  // Send the complete message to the client
            }
            ...
            if (file_count == 0) {
                ...
                int bytesSent = send(new_socket, message, strlen(message), 0);  // Send the message to the client
                memset(message, 0, MAX_RESPONSE_LENGTH);
            } else {
...
int main() {
    ...
    while (1) {
        ...
        if (load_balancer <= 3 || (load_balancer > 9 && load_balancer % 3 == 1)) {
            ...
            printf("Client Forwarded to Mirror 1\n");
            if (fork() == 0) {
            ...
            printf("Client Forwarded to Mirror 2\n");
            if (fork() == 0) {
