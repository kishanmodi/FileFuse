...
void crequest(int new_socket) {
    ...
    while (1) {
        ...
        char **command_tokens;
        int token_count;
        command_tokens = tokenizer(buffer, &token_count);
        if (command_tokens == NULL) {
            close(new_socket);
            return;
        }
        ...
        if (strcmp(command_tokens[0], "dirlist") == 0) {
            if (strcmp(command_tokens[1], "-a") == 0 || strcmp(command_tokens[1], "-t") == 0) {
                ...
                traverse_dir_info(directories, &num_directories, "/home/meet");
            ...
            nftw("/home/meet", Traverse_Files_Recursively, 20, FTW_PHYS);
